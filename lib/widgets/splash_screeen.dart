...
class _AnimatedSplashScreenState extends State<AnimatedSplashScreen> with SingleTickerProviderStateMixin {
  ...
  navigator(screen) {
    var future = widget.future ?? Future.delayed(Duration(milliseconds: w.duration < 100 ? 100 : w.duration));
    future.then((_) {
      try {
        if (screen is String) {
          Navigator.of(_context).pushReplacementNamed(screen);
        } else {
          Navigator.of(_context).pushReplacement(PageTransition(type: w.transitionType, child: screen));
        }
      } catch (msg) {
